...
export const getEnvWarnings = (vars = {}) => {
  ...
  if (vars.VITE_BACKEND && !['api', 'local'].includes(vars.VITE_BACKEND)) {
    warnings.push("VITE_BACKEND should be 'api' or 'local'.")
  }

  if (vars.VITE_API_BASE_URL && vars.VITE_API_BASE_URL !== '/') {
    warnings.push('VITE_API_BASE_URL is not same-origin — the session cookie will not reach the plate viewer iframe.')
  }
