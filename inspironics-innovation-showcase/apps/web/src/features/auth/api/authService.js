 ...
 */
import { authPolicy, env } from '#shared/config'
...
import { createHttpAuthService } from './httpAuthService.js'
...
/** The bundled demo backend: every flow in the browser, over localStorage. */
export const localApi = createAuthService(createLocalAuthRepository())

/** The instance the app uses: the HTTP service for the API backend, else the demo. */
export const api = env.backend === 'api' ? createHttpAuthService() : localApi

export { EMAIL_RE, passwordIssues } from '../model/credentials.js'
export { UnverifiedAccountError }
