...
export default function Lightbox({ items, index, onIndex, onClose }) {
  ...
  // one view per plate opened, for the dashboard and the reader's history
  useEffect(() => {
    if (!item) return
    trackPlateEvent(item, 'view')
    noteView(user?.id, item)
  }, [item, user?.id])

  const download = async () => {
    if (item.contentUrl) {
      // a synced plate: the API streams the original with Content-Disposition: attachment
      trackPlateEvent(item, 'download')
        ...
        >
          ...
          <div className="flex shrink-0 items-center gap-2 border-b border-white/8 px-4 py-3 sm:px-6">
            ...
            <div className="ml-auto flex items-center gap-1.5">
              ...
              <PlateActions item={item} onEvent={(kind) => trackPlateEvent(item, kind)} className="mx-1" />
