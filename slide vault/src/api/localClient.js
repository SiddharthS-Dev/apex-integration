...
/**
 * Reseeds the catalog when the bundled one changes.
 *
 * Seeded tables are written only when their key is absent, so a browser that
 * already ran an older catalog would keep it forever — which is exactly how the
 * app kept showing 32 decks after the real 190 landed in catalog.json. Stamping
 * the catalog's identity and dropping the two derived tables when it moves
 * keeps returning visitors in step. Accounts, sessions and saved searches live
 * under their own keys and are deliberately left alone.
 */
const CATALOG_STAMP = `${CATALOG_SOURCE}:${CATALOG_SIZE}`;

function reconcileCatalog() {
  try {
    if (localStorage.getItem(`${NS}:catalog`) === CATALOG_STAMP) return;
    localStorage.removeItem(`${NS}:Presentation`);
    localStorage.removeItem(`${NS}:PresentationAnalytics`);
    localStorage.setItem(`${NS}:catalog`, CATALOG_STAMP);
  } catch {
    /* private mode — nothing was persisted, so there is nothing to reconcile */
  }
}
...
function entity(name, seedFn, idPrefix) {
  return {
    async list(sort, limit) {
      await latency();
      const rows = sortRecords(table(name, seedFn), sort);
      return limit ? rows.slice(0, limit) : rows;
    },
    async filter(query, sort, limit) {
      await latency();
      const rows = sortRecords(table(name, seedFn).filter((r) => matches(r, query)), sort);
      return limit ? rows.slice(0, limit) : rows;
    },
    async get(id) {
      await latency();
