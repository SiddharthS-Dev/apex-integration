...
/**
 * Catalog used by the local backend so the app is fully explorable without a
 * Dropbox connection. Once a real Dropbox account is connected, syncDropbox
 * replaces these records with the real library.
 */
const CATALOG = [
...
export function buildSeedPresentations() {
  return CATALOG.map(([domain, sub, title, summary, tags], i) => {
