...
import { Presentation, PresentationAnalytics, listAll } from '@/api/entities';
...
export function useLibraryData({ withAnalytics = false } = {}) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const rows = await listAll(Presentation, {
        query: { status: 'active' },
        sort: '-created_date',
      });
      setPresentations(rows);
      if (withAnalytics) {
        setAnalytics(await listAll(PresentationAnalytics, { sort: '-total_views' }));
      }
